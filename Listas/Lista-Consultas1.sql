...
SELECT DISTINCT nome_cliente, nome_agencia
FROM (SELECT DISTINCT nome_cliente, nome_agencia
      FROM deposito
      INTERSECT
      SELECT DISTINCT nome_cliente, nome_agencia
      FROM emprestimo) AS relatorio
...
FROM deposito
WHERE nome_agencia = 'PUC'
  AND nome_cliente NOT IN (SELECT DISTINCT emprestimo.nome_cliente FROM emprestimo WHERE nome_agencia = 'PUC');
